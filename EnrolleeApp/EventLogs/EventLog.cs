...
namespace EnrolleeApp
{
    public partial class EventLog : Form
    {
        ...
        private void tsmiClearLog_Click(object sender, EventArgs e)
        {
            ...
            {
                if (!MainForm.DB.Clear_log())
                {
                    MainForm.DB.Add_log("Очищен локальный журнал", "Удалено строк: " + dgvEvents.Rows.Count.ToString());
                    dgvEvents.Rows.Clear();
                }
            }
            EventLog_Load(this, new EventArgs());
        }
        ...
        private void tsmiHelp_Click(object sender, EventArgs e)
        {
            try
            {
                System.Diagnostics.Process.Start(@"EnrolleeAppHelp.chm");
            }
            catch
            {
